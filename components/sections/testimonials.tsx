...
const Testimonials = () => {
  const getStarType = (rating: number, position: number, index: number) => {
    if (rating >= position)
      return (
    ...
    if (rating >= position - 0.5)
      return (
        <svg width="16" height="16" viewBox="0 0 24 24">
          <defs>
            <clipPath id={`half-${index}-${position}`}>
          ...
          <path
            d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"
            fill="#f59e0b"
            clipPath={`url(#half-${index}-${position})`}
          />
        </svg>
      );
    return (
        ...
        <div className="group overflow-hidden pb-20">
          <div  className="flex gap-4 [animation:var(--animate-marquee)] group-hover:paused">
            ...
            {[...testimonials, ...testimonials].map((e, index) => (
              <div
                key={index}
                className="flex flex-col border rounded-xl border-(--lcnv-teal-pale) px-5 py-4 w-80 shrink-0"
              >
                <div className="flex gap-0.5 py-3">
                  {Array.from({ length: 5 }, (_, i) => i + 1).map(
                    (position) => {
                      const type = getStarType(e.rating, position, index);
                ...
                <span className="text-sm leading-relaxed text-(--lcnv-teal)">
                  {e.text}
                </span>
              </div>
            ))}
